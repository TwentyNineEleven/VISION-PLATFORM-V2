...
export function AppDetailDrawer({
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 z-40 bg-black opacity-60 transition-opacity"
          onClick={onClose}
          aria-hidden="true"
        />
      )}
      ...
      <div
        className={cn(
          'fixed right-0 top-0 z-50 h-full w-full max-w-[640px] bg-card shadow-2xl transition-transform duration-300 ease-out',
      ...
      >
        <div className="flex h-full flex-col overflow-y-auto">
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-4">
            <h2 className="text-xl font-bold text-foreground">App Details</h2>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 px-6 py-6">
            <div className="space-y-6">
              ...
              <button
                ...
                className="w-full rounded-lg bg-primary px-4 py-3 text-base font-semibold text-white transition-all hover:opacity-90 hover:shadow-md"
              >
                Open App
              </button>
              ...
              <div>
                ...
                <ul className="space-y-1 text-sm text-muted-foreground">
                  <li className="flex items-start gap-2">
                    <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-vision-green-700" />
              ...
              <div className="flex flex-wrap gap-2">
                {app.status === 'beta' && (
                  <span className="rounded px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-vision-orange-900 bg-vision-orange-50">
                    Beta
                  </span>
                )}
                ...
                {app.access === 'Requires Upgrade' && (
                  <span className="rounded px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-vision-purple-900 bg-vision-purple-50">
                    Upgrade Required
                  </span>
                )}
              </div>
