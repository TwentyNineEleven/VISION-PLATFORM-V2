 ...
 * Note: Uses (supabase as any) casts until CommunityPulse migrations are applied
...
function dbToEngagement(row: Record<string, unknown>): Engagement {
  return {
    ...
    accessibilityNeeds:
      row.accessibility_needs as Engagement['accessibilityNeeds'],
    ...
    participationModel:
      row.participation_model as Engagement['participationModel'],
    ...
    generatedMaterials:
      row.generated_materials as Engagement['generatedMaterials'],
...
export const communityPulseService = {
  ...
  async updateEngagement(
    ...
    if (updates.aiRecommendations !== undefined)
      dbUpdates.ai_recommendations = updates.aiRecommendations;
  ...
  async getTemplates(organizationId: string): Promise<EngagementTemplate[]> {
    ...
    if (error) throw new Error(error.message);
    return (data || []).map(
      (row: Record<string, unknown>): EngagementTemplate => ({
        id: row.id as string,
        organizationId: row.organization_id as string,
        createdBy: row.created_by as string,
        name: row.name as string,
        description: row.description as string,
        methodSlug: row.method_slug as string,
  ...
  async getMaterials(engagementId: string): Promise<Material[]> {
    ...
    if (error) throw new Error(error.message);
    return (data || []).map(
      (row: Record<string, unknown>): Material => ({
        id: row.id as string,
        engagementId: row.engagement_id as string,
        materialType: row.material_type as Material['materialType'],
        title: row.title as string,
        content: row.content as string | undefined,
        fileUrl: row.file_url as string | undefined,
        version: row.version as number,
        isCustomized: row.is_customized as boolean,
        createdAt: row.created_at as string,
      })
    );
  },
